...
class ReLU():
    ...
    def forward(self,input):#Apply elementwise ReLU to [batch, input_units] matrix
        ...
        self.cache = relu_forward
        return relu_forward
    def backward(self, new_deltaL):#Compute gradient of loss w.r.t. ReLU input
        relu_forward = self.cache
        relu_grad=relu_forward>0
        return new_deltaL*relu_grad
