...
class LeNet5():
    ...
    def __init__(self):
        self.conv1 = Conv(nb_filters = 6, filter_size = 5, nb_channels = 4)
        self.relu1 = ReLU()
        ...
        self.relu3 = ReLU()
        self.fc2 = Fc(row = 40, column = 120)
        self.relu4 = ReLU()
        self.fc3 = Fc(row = 3 , column = 40)
    ...
    def forward(self, X):
        conv1 = self.conv1.forward(X) #(6x32x32)
        act1 = self.relu1.forward(conv1)
        pool1 = self.pool1.forward(act1) #(6x16x16)
        ...
        act3 = self.relu3.forward(fc1)
        
        fc2 = self.fc2.forward(act3) #(1x40)
        act4 = self.relu4.forward(fc2)
    ...
    def backward(self, y_pred, y):
        ...
        deltaL, dW2, db2 = self.conv2.backward(deltaL) #(6x16x116)

        #Distribute error through pool1 by creating a temporary pooling layer
        #of conv1 shape.
        deltaL = self.pool1.backward(deltaL) #(6x32x32)
        #Distribute error through tanh.
